<?php
/**
 * Public Export Schedule to PDF (Grid Layout)
 */
...
// Adjust month index if needed (view sends 1-12 usually from my previous fix, but let's be safe)
// If 0 is sent, it might mean index 0 (Jan). 
// The system generally uses 1-12 for logical months in DB, but array index 0-11 in PHP sometimes.
                ...
                for ($d = 1; $d <= $lastDayNum; $d++) {
                    ...
                    if ($dayData) {
                        if ($dayData['is_holiday']) {
                            ...
                            // RS
                            if (!empty($dayData['rs_morning']) && $dayData['rs_morning'] !== '-') {
                                echo "<div><span class='label'>รส.(เช้า):</span> " . $dayData['rs_morning'] . "</div>";
                            }
                            if (!empty($dayData['rs_afternoon']) && $dayData['rs_afternoon'] !== '-') {
                                echo "<div><span class='label'>รส.(บ่าย):</span> " . $dayData['rs_afternoon'] . "</div>";
                            }
                            // Fallback for legacy data if needed, or if morning/afternoon are somehow missing but rs is set
                            if (
                                (empty($dayData['rs_morning']) || $dayData['rs_morning'] === '-') &&
                                (empty($dayData['rs_afternoon']) || $dayData['rs_afternoon'] === '-') &&
                                !empty($dayData['rs']) && $dayData['rs'] !== '-'
                            ) {
